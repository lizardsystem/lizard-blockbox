...
def generate_csv(request):
    ...
    measures = models.Measure.objects.filter(short_name__in=_selected_measures(request))
    ...
    for measure in measures:
        ...
        summed_minimal_investment_costs += measure.minimal_investment_costs or 0
        summed_maximal_investment_costs += measure.maximal_investment_costs or 0
        summed_investment_costs += measure.investment_costs or 0
    ...
    writer.writerow(
        ["Geselecteerd beschermingsniveau:", "1 / %s" % selected_protection_level]
    )
    ...
    reaches = models.NamedReach.objects.get(name=river).expanded_reaches()

    segments = models.RiverSegment.objects.filter(reach__in=reaches).order_by(
        "location"
    )

    water_levels = (
        _segment_level(
            segment, measures, selected_vertex, selected_year, selected_protection_level
        )
...
def _available_years(request):
    ...
    """
    years = models.VertexValue.objects.all().values_list("year", flat=True).distinct()
...
def _available_rivers(request):
    ...
    selected_year = _selected_year(request)
    rivers = (
        models.NamedReach.objects.filter(vertex__vertexvalue__year__exact=selected_year)
        .distinct()
...
def _available_vertices(request):
    ...
    vertices = models.Vertex.objects.filter(
        named_reaches__name=selected_river
    ...
    vertices = [
        ...
        if models.VertexValue.objects.filter(vertex=vertex, year=selected_year).exists()
    ]
    # ^^^^ TODO: fix query?
    return vertices
...
def _selected_year(request):
    ...
    available_new_years = [year for year in available_years if year.startswith("n")]
    ...
    if request.session[YEAR_SESSION_KEY] not in available_years:
        logger.debug(
            "Year %s is not available anymore, setting it to %s",
            request.session[YEAR_SESSION_KEY],
            desired_if_new,
        )
        request.session[YEAR_SESSION_KEY] = desired_if_new
...
def _selected_vertex(request):
    ...
    if (
        not "vertex" in request.session
        or int(request.session["vertex"]) not in available_vertices_ids
    ):
...
def select_river(request):
    ...
    if river_name not in available_rivers:
        logger.debug(
            "River %s is not available, setting %s instead",
            river_name,
            available_rivers[0],
        )
...
class BlockboxView(MapView):
    ...
    def measures_per_reach(self):
        ...
        for name, measures in reaches.items():
            ...
            reach = {"name": name, "amount": len(measures), "measures": measures}
            result.append(reach)
            ...
            if measure_obj.short_name in available_factsheets:
                try:
                    ...
                    measure["pdf_link"] = reverse(
                        "measure_factsheet", kwargs={"measure": measure_obj.short_name}
                    )
    ...
    def legends(self):
        result_graph_legend = FlotLegend(
            name="Effecten grafiek", div_id="measure_results_graph_legend"
        )
        all_types = models.Measure.objects.all().values_list("measure_type", flat=True)
        labels = []
        ...
        measures_legend = FlotLegend(
            name="Maatregelselectie grafiek", div_id="measures_legend", labels=labels
        )
        ...
        map_measure_results_legend = MapLayerLegend(
            name="Rivieren (kaart)", labels=labels
        )
        ...
        selected_measures_map_legend = MapLayerLegend(
            name="Maatregelen (kaart)", labels=labels
        )
    ...
    def breadcrumbs(self):
        return [
            Action(name="Home", url=reverse("deltaportaal.portalhomepage")),
            Action(name=self.page_title, url=reverse("lizard_blockbox.home")),
        ]
...
class SelectedMeasuresView(UiView):
    ...
    def measures_per_reach(self):
        """Return selected measures, sorted per reach."""
        reaches = defaultdict(list)
        measures = models.Measure.objects.filter(short_name__in=self.selected_names())
    ...
    def breadcrumbs(self):
        return [
            Action(name="Home", url=reverse("deltaportaal.portalhomepage")),
            Action(
                name=self.page_title, url=reverse("lizard_blockbox.selected_measures")
            ),
        ]
...
class BookmarkedMeasuresView(RedirectView):
    ...
    def get_redirect_url(self):
        ...
        if river and river in models.NamedReach.objects.values_list("name", flat=True):
        ...
        if (
            vertex
            and vertex.isdigit()
            and models.Vertex.objects.filter(pk=int(vertex)).exists()
        ):
...
def fetch_factsheet(request, measure):
    ...
    if "+" in measure:
        ...
        filepath = os.path.join(
            DATA_DIR, "factsheets", "{measure}.pdf".format(measure=measure)
        )
...
def _available_factsheets():
    ...
    if os.path.isdir(settings.FACTSHEETS_DIR):
        factsheets = [
            ...
            for i in os.listdir(settings.FACTSHEETS_DIR)
            if i.endswith(".pdf")
        ]
    else:
        factsheets = []
...
def _segment_level(segment, measures, selected_vertex, selected_year, protection_level):
    measures_level = (
        segment.waterleveldifference_set.filter(
            measure__in=measures, protection_level=protection_level
        ).aggregate(ld=Sum("level_difference"))["ld"]
        or 0
    )
    try:
        vertex_level = models.VertexValue.objects.get(
            vertex=selected_vertex, riversegment=segment, year=selected_year
        ).value
    ...
    return {
        ...
        "location_reach": "%i_%s" % (segment.location, segment.reach.slug),
...
def _water_levels(request):
    ...
    cache_key = (
        str(selected_river)
        + str(selected_vertex.id)
        + selected_year
        + selected_protection_level
...
def calculated_measures_json(request):
    ...
    json.dump(
        {"water_levels": water_levels, "measures": measures, "cities": cities}, response
    )
    return response
...
def _included_measures(request):
    ...
    return set(
        models.Measure.objects.filter(
            short_name__in=_selected_measures(request), include__isnull=False
...
def _unselectable_measures(request):
    ...
    return set(
        models.Measure.objects.filter(
            short_name__in=_selected_measures(request), exclude__isnull=False
        ).values_list("exclude__short_name", flat=True)
    )


def _investment_costs(request):
    investment_costs = {"minimum": 0.0, "expected": 0.0, "maximum": 0.0}

    measures = models.Measure.objects.filter(short_name__in=_selected_measures(request))

    for measure in measures:
        investment_costs["minimum"] += measure.minimal_investment_costs or 0
        investment_costs["expected"] += measure.investment_costs or 0
        investment_costs["maximum"] += measure.maximal_investment_costs or 0
...
def toggle_measure(request):
    ...
    available_shortnames = list(
        models.Measure.objects.all().values_list("short_name", flat=True)
    )
    to_remove = []
    for shortname in selected_measures:
        if shortname not in available_shortnames:
            ...
            logger.warn(
                "Removed unavailable shortname %r from selected measures.", shortname
            )
    if to_remove:
    ...
    if measure_id in selected_measures:
        ...
        logger.error("Non-existing shortname %r passed to toggle_measure", measure_id)
...
def _city_locations_json(request):
    ...
    segments_join = (
        models.CityLocation.objects.filter(
            reach=element.reach, km__range=(element.km_from, element.km_to)
        )
    ...
    return [[km, city] for km, city in city_locations.values_list("km", "city")]
...
def _list_measures_json(request):
    ...
    all_types = list(set(measure["measure_type"] for measure in measures))
