...
class ExcelException(Exception):
    ...
    def __str__(self):
        return "Fout in '{filename}', sheet '{sheet}', regel {rownr}: {error}".format(
            filename=(
                os.path.basename(self.path) if self.path is not None else "<onbekend>"
            ),
...
def map_over_sheets(excelpath, function, stdout, *args, **kwargs):
    ...
    def call_function(sheet):
        try:
            ...
            raise ExcelException(path=excelpath, sheet=sheet.name, error=str(e))
...
def fetch_blockbox_data(stdout):
    ...
    print(
        "Note: not removing data dir contents. Do 'docker-compose down -v' if you want "
...
""".format(
    ...
    shapes = [
        shape
        for shape in run_commands_in(
            DATA_DIR, SHAPE_COMMAND, no_extra_output=True
        ).split("\n")
        if shape
    ]
    run_commands_in(DATA_DIR, RM_COMMANDS)

    output = ""
    for shape in shapes:
        json_file = os.path.join(
            JSON_DIR, os.path.basename(shape).replace("shp", "json")
        )

        output += run_commands_in(
            DATA_DIR,
            """
 ogr2ogr -f GeoJSON -s_srs EPSG:28992 -t_srs EPSG:3857 -simplify 0.05 {jsonfile} {shape}
 """.format(
                jsonfile=json_file, shape=shape
            ),
        )
...
def merge_measures_blockbox(stdout):
    ...
    concat_measures = {"type": "FeatureCollection", "features": []}
...
def copy_json_to_media(stdout):
    ...
    for fn in ("measures.json", "kilometers.json"):
        shutil.copyfile(os.path.join(JSON_DIR, fn), os.path.join(MEDIA_DIR, fn))
...
def parse_trajectory_classification_excelfile(excelpath, stdout):
    map_over_sheets(excelpath, parse_trajectory_classification_sheet, stdout)
...
def parse_trajectory_classification_sheet(sheet, stdout):
    for row_nr in range(1, sheet.nrows):
        ...
        models.SubsetReach.objects.get_or_create(
            reach=reach, named_reach=named_reach, km_from=km_from, km_to=km_to
        )
...
def import_city_names_sheet(sheet, stdout):
    for row_nr in range(1, sheet.nrows):
        ...
        models.CityLocation.objects.create(km=int(km), city=city, reach=reach)
...
def build_vertex_dict(row_values):
    ...
    for vertex in row_values:
        ...
        instance, _ = models.Vertex.objects.get_or_create(header=header, name=vertex)
        ...
        instance.year = year
        logger.debug(
            "Added vertex %s (header %s) for year %s",
            instance.name,
            instance.header,
            year,
        )
...
def link_vertices_with_namedreaches():
    for named_reach in models.NamedReach.objects.all():
        ...
        vertices = models.Vertex.objects.filter(
            vertexvalue__riversegment__in=rs
        ).distinct()
...
def import_measure_sheet(sheet, stdout):
    ...
    measure, created = models.Measure.objects.get_or_create(short_name=short_name)
...
def import_measure_row(measure, row_values, rownr, stdout):
    ...
    (location, _, _, difference, reach_slug, difference_250) = row_values
    ...
    try:
        ...
        raise ExcelException(
            rownr=rownr, error="Riviertak {slug} onbekend.".format(slug=reach_slug)
        )
    ...
    try:
        riversegment = models.RiverSegment.objects.get(location=location, reach=reach)
    ...
    try:
        ...
        raise ExcelException(
            error=(
                "Level difference '{difference}' is not "
                "a floating point number.".format(difference=difference)
            ),
            rownr=rownr,
        )
    ...
    if difference_250:
        try:
            ...
            raise ExcelException(
                error=(
                    "Level difference '{difference}' is not "
                    "a floating point number.".format(difference=difference_250)
                ),
                rownr=rownr,
            )
...
def import_measure_table_sheet(sheet, stdout):
    ...
    for row_nr in range(1, sheet.nrows):
        ...
        models.Measure.objects.filter(id=measure.id).update(**default_values)
