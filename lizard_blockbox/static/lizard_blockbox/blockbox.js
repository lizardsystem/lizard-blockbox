(function() {
  ...
  deselectAllMeasures = function() {
    return $.get($('#blockbox-deselect-all-measures').data('deselect-url'), __bind(function(data) {
      updateMeasuresList();
      return this;
    }, this));
  };
  ...
  BlockboxRouter = (function() {
    __extends(BlockboxRouter, Backbone.Router);
  ...
  MeasuresMapView = Backbone.View.extend({
    ...
    initialize: function() {
      ...
      $.getJSON(this.static_url + 'lizard_blockbox/measures.json' + '?' + new Date().getTime(), __bind(function(json) {
        this.measures = JSONTooltip('Maatregelen', json);
        numResponses++;
        if (numResponses === 3) {
      ...
      $.getJSON(this.static_url + 'lizard_blockbox/kilometers.json' + '?' + new Date().getTime(), __bind(function(json) {
      ...
      return $.getJSON(json_url + '?' + new Date().getTime(), __bind(function(data) {
    ...
    render: function(updateRivers, updateMeasures, updateCalculate) {
      ...
      if (updateCalculate) {
        json_url = $('#blockbox-table').data('calculated-measures-url');
        $.getJSON(json_url + '?' + new Date().getTime(), __bind(function(data) {
          this.calculated = data;
          setFlotSeries(data);
          if (updateRivers) {
            this.rivers.destroy();
  ...
  setMeasureGraph = function(control_data, cities_data) {
    ...
    for (_j = 0, _len2 = _ref.length; _j < _len2; _j++) {
      point = _ref[_j];
      _results.push((window.min_graph_value <= (_ref2 = point[0]) && _ref2 <= window.max_graph_value) ? (px = graphx + city_points.xaxis.p2c(point[0]), py = graphy + city_points.yaxis.p2c(point[1]), text = point[2], px > width && text.length > 5 ? px -= text.length * 4 : void 0, showCityTooltip(px, py, text)) : void 0);
    }
    return _results;
  };
